...
export default async function CustomerHomePage() {
  const [restaurants, user] = await Promise.all([
    getStorefrontRestaurants(),
    getCurrentUser(),
  ]);
  const accountHref = user ? appRoutes.account : appRoutes.login;
  const accountLabel = user ? "Профиль и адреса" : "Войти по телефону";
    ...
    <main className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[280px_1fr]">
      <aside className="h-fit rounded-lg border border-border bg-surface p-4">
        ...
        <div className="mt-5 border-t border-border pt-5">
          <div className="text-sm font-medium text-foreground/60">Язык</div>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {restaurants.map((restaurant) => (
            ...
            >
              <div className="relative flex aspect-[16/9] items-center justify-center overflow-hidden rounded-md bg-surface-muted text-sm text-foreground/50">
